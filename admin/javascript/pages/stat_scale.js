...
/* ---------------------------------------------------------
   LOAD SCALE DATA - WITH ROLE FILTER LOGIC
--------------------------------------------------------- */
function loadScaleStats() {
    ...
    if (role === 2) {
        ...
        // include branch users + own library
...
function renderTable(rows) {
    ...
    const scaleNames    = [...new Set(rows.map(r => r.scale_name))].sort();
    ...
    let scaleTotals = {};
    scaleNames.forEach(s => scaleTotals[s] = 0);
    rows.forEach(r => scaleTotals[r.scale_name] += Number(r.total));
    ...
    Object.entries(scaleTotals).forEach(([name, count]) => {
        const bg = getColorForScale(name);
        badgeHTML += `
            <div class="badge-item badge-accent">
                ${name}: ${count}
            </div>
    ...
    scaleNames.forEach(s => hdr += `<th class="header-wrap">${wrapHeader(s)}</th>`);
    ...
    libraryNames.forEach(lib => {
        map[lib] = {};
        scaleNames.forEach(s => map[lib][s] = 0);
    });

    rows.forEach(r => {
        map[r.library_name][r.scale_name] = Number(r.total);
    });
...
$(document).ready(function () {
    ...
    $("#btn-refresh").click(loadScaleStats);
    $("#search-library").on("input", loadScaleStats);

    $("#filter-date-from").on("change", function () {
        applyDateRestrictions();
        loadScaleStats();
    });
    $("#filter-date-to").on("change", loadScaleStats);
